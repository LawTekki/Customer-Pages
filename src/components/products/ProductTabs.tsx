      ...
      <div className="flex flex-col w-full">
        ...
        <div className="hidden md:flex items-center w-full gap-12 overflow-x-auto scrollbar-hide text-base md:text-2xl text-[#808080] font-medium md:font-semibold tracking-[-0.4px] leading-[1.3] mb-2 md:mb-3 whitespace-nowrap">
          {tabs.map((tab, index) => (
            <button
              ...
              className={`whitespace-nowrap px-1.5 md:px-2.5 py-1.5 md:py-2 text-sm md:text-xl hover-scale fade-in ${
                activeTab === tab
        ...
        <div className="flex md:hidden items-center mb-2">
          <button
            ...
            className="flex-shrink-0 w-7 h-7 md:w-9 md:h-9 p-0 md:p-1.5 bg-[#f7eafe] border border-[#e4d0f7] shadow-lg hover:bg-[#e9d6fa] rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#6B047C] flex items-center justify-center button-pulse fade-in"
...
const MobileFilterOverlay = ({ tabs, activeTab, setActiveTab, onTabClick, onClose, ProductFilters }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black bg-opacity-30">
      <div ref={overlayRef} className="bg-white rounded-lg shadow-2xl mt-4 w-[90vw] max-w-xs p-4 flex flex-col gap-3 slide-in max-h-[80vh] overflow-y-auto">
        <div className="flex flex-col gap-1 mb-2">
          {tabs.map((tab, index) => (
            <button
              key={tab}
              onClick={() => { setActiveTab(tab); onTabClick(tab); }}
              className={`text-left px-3 py-2 rounded-lg text-base font-medium transition-colors hover-scale fade-in ${
                activeTab === tab
