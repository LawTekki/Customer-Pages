      ...
      <div className="w-full max-w-[1416px] max-md:max-w-full">
        <div className="gap-5 flex max-md:flex-col max-md:items-stretch">
          ...
          <main className="w-[81%] ml-5 max-md:w-full max-md:ml-0 max-md:px-4">
            <div className="flex flex-col font-medium mt-10 max-md:mt-4 max-md:max-w-full">
              <div className="flex max-md:flex-col max-md:gap-2">
                <div className="flex flex-col gap-2 w-[120px] max-md:w-full max-md:flex-row max-md:justify-between max-md:gap-0 max-md:mb-0 max-md:pb-2 max-md:border-b max-md:border-[#E6E6E6]">
                  <button
                    onClick={() => setSelectedButton('faqs')}
                    className={`relative text-base font-medium px-4 py-2 hover:text-[#6B047C] transition-colors max-md:text-sm max-md:px-2 max-md:py-1.5 ${selectedButton === 'faqs' ? 'text-[#6B047C] bg-[#F9F5FA]' : 'text-[#808080]'}`}
                  >
                  ...
                  <button
                    ...
                    className={`relative text-base font-medium px-4 py-2 hover:text-[#6B047C] transition-colors max-md:text-sm max-md:px-2 max-md:py-1.5 ${selectedButton === 'contact' ? 'text-[#6B047C] bg-[#F9F5FA]' : 'text-[#808080]'}`}
                  >
                    {selectedButton === 'contact' && <span className="absolute left-0 top-0 h-full w-1 bg-[#6B047C] rounded-r max-md:hidden"></span>}
                    Contact us
                  </button>
                </div>
                <div className="leading-[1.3] ml-[150px] flex-1 max-md:max-w-full max-md:mt-2 max-md:ml-0">
                  <h1 className="text-[#1A011E] text-2xl font-medium tracking-[-0.48px] max-md:text-lg max-md:leading-snug">
